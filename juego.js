...
var instrucciones = ["utlizar las flechas para mover las piezas", "Ordenar hasta alcanzar la imagen objetivo"];
...
function chequearSiGano() {
  ...
  var control = 1;
  var solucion = 10;
  var len = grilla.length;
  for(var i=0; i < len; i++){
    for(var j=0; j < len; j++){
      if(grillaOriginal[i][j] != grilla[i][j]){
        control = 1;
      } else {
        control ++
      }
  ...
  return control === solucion ? true : false;
}

// Implementar alguna forma de mostrar un cartel que avise que ganaste el juego
function mostrarCartelGanador() {
      alert("Felicidades, has ganado! lo lograste en" + " " + movimientos.length + " " + "moviemientos")
